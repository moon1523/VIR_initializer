...
int View_Coordinates(int argc, char** argv)
{
	...
	Read_Point_Clouds(point_clouds, point_clouds_color);
	...
	while(!window_est.wasStopped())
	{
		...
		center_widget.setRenderingProperty(cv::viz::OPACITY, 0.5);
                      ...
                      string pathName)
{
	...
	for (size_t i=0;i<cam_files.size();i++) 
	{
		...
		if ( cam_files[i].find("000") != string::npos) { // kinect
			...
			cv::Matx33d K = cv::Matx33d( 1000, 0, 600,  0, 1000, 300,  0, 0, 1);
...
{
	char buffer[128];
	string cmd = "find "+pathName+" -name \"*_tf.ply\"";
	FILE* pipe = popen(cmd.c_str(), "r");
	if (!pipe) cerr << "popen faeild" << endl;
